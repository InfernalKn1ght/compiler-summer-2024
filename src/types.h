...
namespace AST {
	...
	enum BinaryOperator { PLUS = '+',
						  ...
						  LEFT_BRACKET = '(',
						  RIGHT_BRACKET = ')',
						  WHILE=100, //Приравниваю чтобы не возникало дублирования значений
						  IF=101,
						  ELSE=102};
	...
	class Stmt: public Expr {
		...
		Stmt(std::unique_ptr<EBinOp> tree, std::unique_ptr<Stmt> next = nullptr) : 
			tree(std::move(tree)), next(std::move(next)) {}
		void print(const std::string& prefix="", bool isLeft=false) const override;
	};
}
